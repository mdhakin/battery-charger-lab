  ...
  while (running) {
    ...
    if (command == "help") {
      ...
      if (!(input >> level)) {
        ...
        battery.setChargeLevel(static_cast<std::uint8_t>(level));

        std::cout << "Charge level set to " << level << "%\n";
      }
      ...
      std::cout << "Battery State: " << stateToString(battery.getState()) << '\n';
    } else if (command == "state") {
      std::cout << "Battery State: " << stateToString(battery.getState()) << '\n';

      std::cout << "Installed: " << (battery.isInstalled() ? "true" : "false") << '\n';

      std::cout << "Charge Level: " << static_cast<int>(battery.getChargeLevel()) << "%\n";

      std::cout << "Fully Charged: " << (battery.isFullycharged() ? "true" : "false") << '\n';
      ...
      if (!(input >> tick)) {
        ...
        std::cout << "Current Time " << currentTimeMs << " Ms"
                  << "\n";
      }
